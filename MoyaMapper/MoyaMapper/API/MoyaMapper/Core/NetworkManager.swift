...
class NetworkManager<T> where T: Mappable {
    ...
    // 用来处理只请求一次的栅栏队列
    private let barrierQueue = DispatchQueue(label: "nm.NetworkManager", attributes: .concurrent)
...
extension NetworkManager {
    ...
    private func handleResponseData<R: TargetType & MoyaAddable>(_ isList: Bool, type: R, data: Data) throws -> (ModelResponse<T>?, ListResponse<T>?) {
        ...
        if isList {
            ...
            if !ResponseCode.successResponseStatus.contains(temp.code) {
    ...
    private func createProvider<T: TargetType & MoyaAddable>(type: T, test: Bool) -> MoyaProvider<T> {
        ...
        let provider = MoyaProvider<T>(plugins: [activityPlugin, NetworkLoggerPlugin()])
        return provider
    }
...
extension NetworkManager {
    ...
    private func cleanRequest<R: TargetType & MoyaAddable>(_ type: R) {
        ...
        case let .requestParameters(parameters, _):
            let key = type.path + parameters.description
            barrierQueue.sync(flags: .barrier) {
                fetchRequestKeys.removeFirst(where: { $0 == key })
            }
        default:
            // 不会调用
            ()
